/* Bright editorial Chart.js — after Chart.js, before app.js */
(function () {
  const ACCENT = '#14b8a6';
  const GRID = 'rgba(12,10,9,0.06)';
  const TICK = '#a8a29e';
  ...
  try {
    ...
    OrigChart.defaults.plugins.legend.labels.color = '#57534e';
    OrigChart.defaults.plugins.tooltip.backgroundColor = 'rgba(12,10,9,0.92)';
    OrigChart.defaults.plugins.tooltip.titleColor = '#fafaf9';
    OrigChart.defaults.plugins.tooltip.bodyColor = '#e7e5e4';
    ...
    OrigChart.defaults.elements.point.hoverRadius = 6;
  ...
  function Chart(ctx, config) {
    try {
      ...
      if (config && config.options && config.options.plugins && config.options.plugins.legend && config.options.plugins.legend.labels) {
        config.options.plugins.legend.labels.color = '#57534e';
      }
        ...
        datasets.forEach(function (ds) {
          ...
          if (Array.isArray(ds.backgroundColor)) {
            ds.backgroundColor = ds.backgroundColor.map(function (c) {
              ...
              if (c === '#94a3b8') return '#a8a29e';
              return c;
            });
          }
          if (ds.borderColor === '#94a3b8') ds.borderColor = '#a8a29e';
